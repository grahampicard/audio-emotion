...
# user-created files
from models.cnn_mfcc_3_seconds import CNN_simple_3s_32k
...
def test(model, dataloader, device, args):
    ...
    output_df.to_csv(f'./data/processed/mfcc/cnn-{args.model}-3s_32k-test-results.csv', index=False)
...
def save_confusion(pred, true, file_name, annotations=True):
    ...
    cf.to_csv(f'./data/processed/mfcc/cnn-{args.model}-3s_32k-test-confusion.csv')
...
if __name__ == "__main__":
    ...
    # Waveform
    # Load training data
    train_features, train_labels, valid_features, valid_labels, test_features, test_labels = load_section_level_stft(preprocessing='mfcc', label_type='hard-labels')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (curr_valid_loss < min_valid_loss):
            ...
            if (args.save_model):
                dir_path = "./data/processed/mfcc/"
                if not os.path.exists(dir_path): os.makedirs(dir_path)
                torch.save(curr_model, f"./data/processed/mfcc/cnn-{args.model}-3s_32k.pt")
            print("Found new best model, saving to disk!")

    best_model = torch.load(f"./data/processed/mfcc/cnn-{args.model}-3s_32k.pt")
    pred, true = test(best_model, test_loader, device, args)
    save_confusion(pred, true, f"./data/processed/mfcc/cnn-{args.model}-3s_32k-confusion.csv", annotations=False)
    pd.DataFrame(full_log).to_csv(f"./data/processed/mfcc/cnn-{args.model}-3s_32k-training-loss.csv", index=False)