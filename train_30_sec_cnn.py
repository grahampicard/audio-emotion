...
# user-created files
from models.cnn_30_seconds import CNN_simple_30s_32k
from models.cnn_30_seconds import CNN_dev_30s_32k
...
if __name__ == "__main__":
    ...
    # Load training data
    train_features, train_labels, test_features, test_labels = load_stft_data(split=.8, sample_length=30)
    ...
    if args.model == 'simple':
        model = CNN_simple_30s_32k().to(device)
    if args.model == 'dev':
        model = CNN_dev_30s_32k().to(device)
    ...
    if (args.save_model):
        torch.save(model.state_dict(), f"./data/processed/cnn-{args.model}-30s_32k.pt")
