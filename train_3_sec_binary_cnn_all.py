...
def test(model, dataloader, device, args, label):
    ...
    output_df.to_csv(f'./data/processed/stft-binary/cnn-{label}-{args.model}-3s_32k-test-results.csv', index=False)

    cf = pd.DataFrame(confusion_matrix(y_preds, y_true))
    cf.index.name = 'y_preds'
    cf.to_csv(f'./data/processed/stft-binary/cnn-{label}-{args.model}-3s_32k-test-confusion.csv')
...
if __name__ == "__main__":
    ...
    hard_labels = pd.read_csv('data/interim/expanded-3secondsegments/labels/hard-labels.csv', index_col=[0,1]).columns
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if (curr_valid_loss < min_valid_loss):
                ...
                if (args.save_model):
                    ...
                    torch.save(curr_model, f"./data/processed/stft-binary/cnn-{label}-{args.model}-3s_32k.pt")
                print("Found new best model, saving to disk!")

        best_model = torch.load(f"./data/processed/stft-binary/cnn-{label}-{args.model}-3s_32k.pt")
        pred, true = test(best_model, test_loader, device, args, label)
        pd.DataFrame(full_log).to_csv(f"./data/processed/stft-binary/cnn-{label}-{args.model}-3s_32k-training-loss.csv", index=False)
